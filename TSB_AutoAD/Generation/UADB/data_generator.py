...
class DataGenerator():
    ...
    def generator(self, X=None, y=None,
        ...
        if self.dataset is None:
            ...
            # data = np.load('../UADB/datasets/33_shuttle.npz', allow_pickle=True)
            # X = data['X']
            # y = data['y']
            # print("supported:",X.shape, y.shape)
            #filepath = '../UADB/datasets/Yahoo_A1real_1_data.out'
            # df = pd.read_csv(self.dataset).dropna().to_numpy()
            # X = df[:,0].astype(float).reshape(-1,1)
            # y = df[:,1].astype(int)
            df = pd.read_csv(self.dataset).dropna()
        ...
        #TODO : check data augmentation and trimming
        ...
        if realistic_synthetic_mode is not None:
            ...
            if realistic_synthetic_mode == 'dependency':
                dataset_dict = np.load(f'datasets/dependency_outlier_large.npz', allow_pickle=True)
        ...
        X_train, X_test, y_train, y_test = X, X[int(0.3*len(y)):, :], y, y[int(0.3*len(y)):] 
