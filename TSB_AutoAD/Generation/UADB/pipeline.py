...
class Pipeline(object):
    ...
    def boost_co_train(self):
        ...
        if self.config.experiment_type == 'UADB':
            for rep in range(10):
                self.train()
                _, labels, scores = self.decision_function_co_train(rep)
                self.tab_data.update_pseudo_label(scores)
                self.tab_data.build_train_dataloaders()
                self.tab_data.build_entire_dataloader()
                # print('scores---**---:', scores)
        ...
        elif self.config.experiment_type == 'Mean_cascade' or self.config.experiment_type == 'STD_cascade':
            for rep in range(10):
                self.train()
                ...
                self.tab_data.build_train_dataloaders()
                self.tab_data.build_entire_dataloader()                   
        elif self.config.experiment_type == 'Mean' or self.config.experiment_type == 'STD':
            self.train()
