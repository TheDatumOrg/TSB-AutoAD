...
class InjectAnomalies:
    ...
    def inject_anomalies(self,
        ...
        if anomaly_type == 'spikes':
            ...
            spike_labels = (np.abs(spikes) >= sorted(spikes)[-10]).astype(int)

        elif anomaly_type == 'contextual':
            anomaly = self.inject_contextual_anomaly(
                T[:, self.anomaly_start:self.anomaly_end], scale=scale)
            timeseries_with_anomalies[:, self.anomaly_start:self.anomaly_end] = anomaly * correlation_vec[:,None] + T[:,
                                                                                         self.anomaly_start:self.anomaly_end]
...
def gen_synthetic_performance_list(data, label, Det_pool):
    ...
    for det in Det_pool:
        ...
        data_train = data[:int(0.2*len(data)), :].reshape(-1, 1)
    ...
    for score in score_list:
        synthetic_performance_list.append(average_precision_score(label, score))
