...
from .utils import mallows_kendall as mk
...
def simple_agg(ranks):
    ranks_rev = ranks[::-1]
    ranks_rev = np.argsort(ranks_rev)
    preds_agg = np.sum(ranks_rev, axis=0)
    preds = np.argsort(preds_agg, axis=0)[::-1]
    return preds
...
rank_agg = {
    "Kemeny": kemeny,
    "Trimmed_kemeny": trimmed_kemeny,
    "Borda": borda, 
    "Partial_borda": partial_borda, 
    "Trimmed_borda": trimmed_borda, 
    "Trimmed_partial_borda": trimmed_partial_borda,
    "MIM": minimum_influence
}


def run_rank_agg(variant, ranks):
    autoad = rank_agg.get(variant)
    if autoad:
        return autoad(ranks)
    else:
